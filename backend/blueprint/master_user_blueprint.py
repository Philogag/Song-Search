...
master_user_blueprint = Blueprint(
    name="master_user", import_name=__name__, url_prefix="/master-user"
)
...
def route_get_current_user_info():
    ...
    try:
        carrier.result = get_user_info(user_id=get_current_user_id())
...
def route_get_user_list_for_management():
    ...
    try:
        ...
        carrier.result = get_user_list_for_management(params=PaginationParams(**data))
    except BusinessError as err:
        logging.exception(err)
        carrier.push_exception(err)
    return jsonify(carrier.to_dict(to_camel=True))
