...
class MenuRepository(BasicRepository):
    ...
    def get_current_menu(
        cls, current_user_id: str, current_role_id: str
    ) -> List[MenuTreeVm]:
        """根据用户id和用户当前role生成菜单"""
        sql = """
        with menu_tree as (
    ...
    def update_menu_item(
        cls, data: MenuEm, transaction: Transaction, col_list: Iterable[str] = None
    ) -> None:
    ...
    @classmethod
    def get_child_menu(
        cls, menu_name: str, current_user_id: str, current_role_id: str
    ) -> List[MenuTreeVm]:
        sql = """
