...
class RoleRepository(BasicRepository):
    ...
    def get_all_role(cls) -> List[RoleEm]:
        return cls._fetch_all(
            sql="SELECT * FROM st_role order by handled_at asc",
            model_cls=RoleEm,
            params={},
        )
    ...
    def update_menu_item(
        cls, data: RoleEm, transaction: Transaction, col_list: List[str] = None
    ):
    ...
    def delete_role_by_id(cls, role_id: str, transaction: Transaction):
        return cls._delete_entity_by_id(
            entity_cls=RoleEntity, transaction=transaction, entity_id=role_id
        )
