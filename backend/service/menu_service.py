...
def get_current_menu_by_user() -> List[MenuTreeVm]:
    ...
    menu_list = MenuRepository.get_current_menu(
        current_user_id=current_user_id, current_role_id=current_role.id
    )
...
def create_or_update_menu_item(
    data: MenuEm, transaction: Transaction, do_update=False
) -> str:
    if not do_update:
