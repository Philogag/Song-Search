...
from backend.data.system_enum import EnumDataPermissionCategory, EnumHandlerCategory
...
def get_user_current_role(user_id: str) -> Optional[UserRoleMapEm]:
...
def set_user_role_list(user_id: str, role_id_list: List[str], transaction: Transaction):
    ...
    exist_roles_id = set(
        map(lambda x: x.id, UserRoleMapRepository.fetch_roles_by_user_id(user_id))
    )
...
def create_or_update_role_item(
    data: RoleEm, transaction: Transaction, do_update
) -> str:
    if not do_update:
...
def delete_role_item_by_id(role_id: str, transaction: Transaction):
    ...
    for data_permission in data_permissions:
        DataPermissionRepository.delete_data_permission_with_detail(
            data_permission_id=data_permission.id, transaction=transaction
        )
